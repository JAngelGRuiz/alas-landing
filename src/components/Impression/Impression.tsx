    ...
    <div className="py-6 px-3 sm:px-10 rounded-bl-[60px] rounded-br-[60px]">
      <article className="sm:h-dvh w-ful gap-y-10 l flex flex-col">
        <section className="min-w-full flex flex-col items-center flex-auto">
          <div className="flex-1 flex flex-col justify-center items-center h-full gap-y-9 sm:gap-y-4 text-center">
            <div className="font-extrabold text-4xl md:text-5xl [text-wrap:balance] text-primary-foreground flex flex-col items-center">
              ...
              <span className="text-indigo-500 inline-flex flex-col h-[calc(theme(fontSize.4xl)*theme(lineHeight.tight))] md:h-[calc(theme(fontSize.5xl)*theme(lineHeight.tight))] overflow-hidden">
                <ul className="block animate-text-slide-6 sm:text-left leading-tight [&_li]:block">
                  {listOfCourses.map((course) => (
                    <li className="text-center" key={course}>
                      {course}
                    </li>
                  ))}
            ...
            <h1 className="text-xl text-muted-foreground leading-tight">
              ¡Estudia donde y lo que quieras! <br /> Nosotros te damos alas
              para volar
            </h1>
            <Link
        ...
        <section className="flex-1 w-full py-10 sm:py-0 border border-primary rounded-3xl min-h-[221px] flex flex-col sm:flex-row items-center justify-center space-y-24 sm:space-y-0">
